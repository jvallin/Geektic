GEEKTIC
=====

TP sur pour J2EE

L'utilisateur peut :
	- soit rechercher par pseudo (ou bout de pseudo)
	- soit rechercher par sexe

La recherche par sexe + interets ne fontionne pas.

Chaque geek possède un pseudo, un nom, un prenom, un sexe et un nombre de vues qui augmente à chaque affichage de la page 
(BUG : si l'utiliseteur "bourine" la touche F5, le nombre de vues augmente a chaque fois)



